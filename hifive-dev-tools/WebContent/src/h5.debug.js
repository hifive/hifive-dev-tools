	...
	(function() {
		...
		var baseLogController = {
			...
			'.showFunction click': function(context, $el) {
				...
				setTimeout(function() {
					...
					var $activeList = debugCtrl.$find('.controller-detail .' + tabCls
							+ ' .method-list');
		...
		var traceLogController = {
			...
			_createLogHTML: function(logArray) {
				...
				for ( var i = 0, l = logArray.length; i < l; i++) {
					...
					var part = '<li class="' + logObj.cls + '" data-h5debug-logindex="' + i + '">'
							...
							+ logObj.promiseState + '</span>' + '<span class="message '
							+ logObj.cls + '">' + logObj.message + '</span></li>';
					// フィルタにマッチしているか
			...
			'button.filter-clear click': function(context) {
				...
				this.refresh();
				this.$find('input.filter[type="text"],.filter-show,.filter-hide').removeAttr(
						'disabled');
		...
		var debugController = {
			...
			__construct: function(context) {
				...
				this.__meta._controllerDebugController.rootElement = this
						.$find('.debug-controller');
					...
					.createInterceptor(
							function(invocation, data) {
								...
								if (preTarget && preTarget !== target) {
									var logObj = createLogObject(target, target.__name + '#'
											+ fName, cls, 'BEGIN', '', target.__name,
											preTarget._h5debugContext.indentLevel);
								...
								// ターゲットのログ
								var logObj = createLogObject(target, fName, cls, 'BEGIN', '',
										target.__name, indentLevel);
								data.logObj = logObj;
								...
								var wholeLog = createLogObject(target, target.__name + '#' + fName,
										cls, 'BEGIN', '', target.__name, wholeTraceLogsIndentLevel);
