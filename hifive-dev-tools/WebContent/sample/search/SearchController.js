(function() {
	...
	var searchController = {
		...
		'.form-search  submit': function(context) {
			...
			var promise = this._searchLogic.search(keyword);

			this.$find('.search-query').blur();

			promise.done(this.own(function(data) {
				this.totalCount = data.pageInfo.totalResults;
		...
		'.search-result-lists [scroll]': function(context, $el) {
			...
			var nextPageToken = this._nextPageToken;
			if (!nextPageToken) {
				// もう次の結果は無い
				this.$find('.emptyList').remove();
				return;
			}
			var promise = this._searchLogic.next(keyword, nextPageToken);
